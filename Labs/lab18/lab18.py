...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(re.compile("C ?S ?\d\d\dR?"), post))
...
def match_time(text):
    ...
    ['2:00']
    """
    reg = re.compile(r"\s((?:(?:[0-1]?[0-9])|(?:2[0-4])):(?:(?:[0-5][0-9])|60)(?:AM|PM)?)")
    return re.findall(reg, text)
...
def area_codes(text):
    ...
    []
    """
    reg = re.compile(r'\b\(?(\d\d\d)\)?(?:[- ]?)\d\d\d[- ]?\d\d\d\d\b')
    return re.findall(reg, text)
...
def most_common_code(text):
    ...
    """
    """*** YOUR CODE HERE ***"""
    areas = area_codes(text)
    area_counts = {}
    for area in areas:
        if area not in area_counts.keys():
            area_counts[area] = 1
        elif area in area_counts.keys():
            area_counts[area] += 1
